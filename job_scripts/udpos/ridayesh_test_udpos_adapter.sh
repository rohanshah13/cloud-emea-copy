...
TASK='udpos'
# LANGS="hu"
LANGS="am,ar,bh,bn,bxr,cdo,cs,de,el,en,es,et,eu,fa,fi,fr,gn,hi,ht,hu,hy,id,ilo,is,ja,jv,ka,ko,kv,la,lv,mhr,mi,my,myv,pt,qu,ru,se,sw,tk,tr,vi,wo,xmf,zh,zh_yue" 
...
# ADAPTERS_LANGS=( "hy" "hi" "fa" "ja" "is" "fr" "my" "ht" "gn" "myv" "mhr" )
# LANG_ADAPTER_NAMES=( "hy/wiki@ukp" "hi/wiki@ukp" "fa/wiki@ukp" "ja/wiki@ukp" "is/wiki@ukp" "fr/wiki@ukp" "my/wiki@ukp" "ht/wiki@ukp" "gn/wiki@ukp" "myv/wiki@ukp" "mhr/wiki@ukp" )
...
OUTPUT_DIR="$OUT_DIR/${TASK}/my-${MODEL}-MaxLen${MAX_LENGTH}_${TASK_ADAPTER_NAME}_NO_LANG_ADAPTER_train_${TRAIN_LANGS}/"
mkdir -p $OUTPUT_DIR
for SEED in 1 2 3
do
...
nohup time python third_party/ridayesh_run_tag.py \
  --predict_save_prefix "" \
  --calc_weight_step 0 \
  ...
  --load_lang_adapter "" \
  --language "None" >> $OUTPUT_DIR/detailed_train_en.log
done
