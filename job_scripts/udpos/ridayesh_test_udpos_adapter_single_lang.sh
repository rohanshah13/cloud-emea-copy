...
LANG_ADAPTER_NAMES=( "hi/wiki@ukp" )
ADAPTERS_LANGS=( "hi" )
...
OUTPUT_DIR="$OUT_DIR/${TASK}/my-${MODEL}-MaxLen${MAX_LENGTH}_${TASK_ADAPTER_NAME}_MARATHI_HIDNI_TASK_train_${TRAIN_LANGS}/"
mkdir -p $OUTPUT_DIR
for SEED in 1 2 3
do
...
nohup time python third_party/ridayesh_run_tag_unmodified.py \
  ...
  --load_lang_adapter $LANG_ADAPTER_NAMES \
  --language $ADAPTERS_LANGS >> $OUTPUT_DIR/detailed_train_en.log
done
