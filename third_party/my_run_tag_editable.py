...
def evaluate(args, model, tokenizer, labels, pad_token_label_id, mode, prefix="", lang="en", lang2id=None, print_result=True, adapter_weight=None, lang_adapter_names=None, task_name=None, calc_weight_step=0):
  ...
  for batch in tqdm(eval_dataloader, desc="Evaluating"):
    ...
    if args.get_attr:
      ...
      for tar_layer in range(12):
        ...
        att_all = att[:, 0:input_len, 0:input_len]
        ...
        if example_head_attentions is None:
          entropies = []
          for i in range(att_all.size()[0]):
            head_entropies = []
            for j in range(att_all.size()[1]):
              head_entropies.append(torch.distributions.Categorical(att_all[i,j]).entropy().item())
            entropies.append(sum(head_entropies)/len(head_entropies))
          entropies = torch.FloatTensor(entropies)
          example_head_attentions = entropies.unsqueeze(0)
        else:
          entropies = []
          for i in range(att_all.size()[0]):
            head_entropies = []
            ...
            entropies.append(sum(head_entropies)/len(head_entropies))
          entropies = torch.FloatTensor(entropies)
          example_head_attentions = torch.cat((example_head_attentions, entropies.unsqueeze(0)), dim=0)
      ...
      head_attentions = example_head_attentions if head_attentions is None else torch.add(head_attentions, example_head_attentions)
      if counter == 100:
        break
      continue
    with torch.no_grad():
  ...
  if args.get_attr:
    ...
    head_attentions = head_attentions/counter
    # logger.info(f'Head Importances = {head_importances}')
    logger.info(f'Head Attentions = {head_attentions}')
    ...
    torch.save(head_attentions, os.path.join(args.output_dir,f'{mode}_{lang}_s{args.seed}_entropies_100.pt'))
    return None, None
  ...
  if adapter_args.load_lang_adapter or load_lang_adapter:
      if load_lang_adapter is None:
          ...
          if adapter_args.language == 'topk':
            ...
            filename = f'weights/panx/{args.predict_langs}/top3_s{args.seed}'
            languages = []
    ...
    for lang in args.predict_langs.split(','):
      ...
      if not args.adapter_weight and not args.lang_to_vec:
        ...
        if args.adapter_weight == 'equal':
          ...
          filename = f'weights/panx/{lang}/weights_s{args.seed}'
          with open(filename) as f:
