...
def squad_convert_example_to_features(example, max_seq_length, doc_stride, max_query_length, is_training, lang2id, bpe_dropout=0):
    ...
    truncated_query = tokenizer.encode(example.question_text, add_special_tokens=False, max_length=max_query_length, dropout=bpe_dropout, truncation=True)
    ...
    while len(spans) * doc_stride < len(all_doc_tokens):
        # logger.info(span_doc_tokens)
        counter += 1
        # if len(span_doc_tokens) == 0:
            # logger.info('FFFFFFFFFFFFFFFFFF')
        # logger.info(f'Examples = {example.doc_tokens}')
        # logger.info(f'Counter = {counter}')
        # logger.info(f'Span Doc Tokens = {span_doc_tokens}')
        # logger.info(f'Truncated query = {truncated_query}')
        # logger.info(f'Doc Stride = {doc_stride}')
        # logger.info(f'len(all_doc_tokens) = {len(all_doc_tokens)}')
        # logger.info(f'stride={max_seq_length - doc_stride - len(truncated_query) - sequence_pair_added_tokens}')
            # exit()
